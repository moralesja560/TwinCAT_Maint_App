...
class hilo1(threading.Thread):
	...
	def run(self):
		...
		plc.open()
		var_handle46_1 = plc.get_handle('.VS_Pumproom_Water_Pressure_PSI')
		while True:
			try:
				#sensor
...
def write_log(cell1):
	...
	ruta = str(mis_docs)+ r"\pump_room.txt"
	pd_ruta = str(mis_docs)+ r"\pump_room_df.csv"
	...
	if file_exists == True:
		with open(ruta, "a+") as file_object:
			...
			if len(data) > 0 :
				file_object.write("\n")
				file_object.write(f" timestamp {dt_string}: Pressure: {cell1}")
	else:
		with open(ruta,"w+") as f:
				f.write(f" timestamp {dt_string}: Pressure: {cell1}")
	...
	new_row = {'timestamp' : [dt_string], 'Pressure' : [cell1]}
...
pd_dict = {'timestamp' : ['dummy'], 'Pressure' : ['dummy']}
...
if __name__ == '__main__':
	...
	try:
		...
		print(ams_net_id)
		pyads.close_port()
		plc=pyads.Connection('10.65.68.120.1.1', 801, '10.65.68.120')
	except:
